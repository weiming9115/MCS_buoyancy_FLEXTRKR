...
def write_histogram_regions(Buoy_TOT, prec_gpm, bins_bl, geo_info):
    ...
    # deal with nan values if any
    prec_gpm_mask = prec_gpm_1d[~np.isnan(prec_gpm_1d)]
    ...
    for n in range(len(bins_bl)-1):
        ...
        prec_gpm_sum[n] = np.sum(prec_gpm_mask[idx])
...
if __name__ == '__main__':
    ...
    ds.to_netcdf(out_dir / 'buoy_precipitation_multiregions.{}.nc'.format(year))
