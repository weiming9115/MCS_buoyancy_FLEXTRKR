...
if __name__ == '__main__':
    ...
    data_omega = xr.open_dataset(buoy_dir / 'MCS_FLEXTRKR_tropics_buoyancy.merged.nc')
    data_omega_ltmean = (data_omega.thetae_sat_lt).rename('omega_ltmean')
    ...
    # rename omega_ltmean to thetae_bl
    data_bins_duration = data_bins_duration.rename_vars({'omega_ltmean_sum': 'thetae_sat_lt_sum'})

    out_dir = Path('/scratch/wmtsai/temp_mcs/output_stats/cape_subsat_hist')
    data_bins_duration.to_netcdf(out_dir / 'hist2d_cape_subsat_dtype.{}.{}.thetae_sat_lt.full.nc'.format(year,sampling_opt))
    print('hist2d_cape_subsat_dtype.{}.{}.thetae_sat_lt.full.nc'.format(year,sampling_opt))
