...
if __name__ == '__main__':
    ...
    buoy_samples = np.zeros((2,len(bins_bl)-1)) # non-MCS and MCS grids
    prec_gpm_sum = np.copy(buoy_samples)
    ...
    for month in np.arange(6,9):
        ...
        # get nonMCS-assocaited and MCS-associated grids
        prec_gpm_nonmcs = prec_gpm.where(mcs_tag == 0)
        Buoy_TOT_nonmcs = Buoy_TOT.where(mcs_tag == 0)
        prec_gpm_mcs = prec_gpm.where(mcs_tag > 0)
        Buoy_TOT_mcs = Buoy_TOT.where(mcs_tag > 0)        

        # 3. sampling by BL bins over the region
        (b_samples, gpm_sum) = write_histogram_regions(Buoy_TOT_nonmcs
                                                            , prec_gpm_nonmcs, bins_bl, geoinfo)
        buoy_samples[0,:] += b_samples
        prec_gpm_sum[0,:] += gpm_sum

        (b_samples, gpm_sum) = write_histogram_regions(Buoy_TOT_mcs
                                                            , prec_gpm_mcs, bins_bl, geoinfo)
        buoy_samples[1,:] += b_samples
        prec_gpm_sum[1,:] += gpm_sum

    # writeout data
    ds = xr.Dataset(data_vars = dict(samples = (['grid_type','BL_bins'], buoy_samples),
                                     prec_gpm_sum = (['grid_type','BL_bins'], prec_gpm_sum)),
                    coords = dict(BL_bins = (['BL_bins'], bins_bl[:-1]),
                                  grid_type = (['grid_type'], ['non_MCS','MCS']),
                                  region = (['region'], list(region_name))),
                    attrs = dict(description = 'buoy-precipitation relationship in tropical regions',
                                 bins_unit = 'degree Kelvin',
                                 timeframe = 'Summertimes. June-July-August',
                                 geolocation = '[28.5,36.5,270,278]')
                   )  

    out_dir = Path('/scratch/wmtsai/temp_mcs/output_stats/buoy_precip_1D_multiregions/{}'.format(region_name))
    os.system('mkdir -p {}'.format(out_dir))
    ds.to_netcdf(out_dir / 'buoy_precipitation_{}.{}.JJA.nc'.format(region_name, year))
    print('buoy_precipitation_{}.{}.JJA.nc ...completed'.format(region_name, year))
