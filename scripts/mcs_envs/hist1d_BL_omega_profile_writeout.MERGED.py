...
def BL_omega_hist(data_merged_phase):
    
    # bins for BL_CAPE and BL_SUBSAT
    bins_BL = np.arange(-30,10,0.25)
    bins_samples = np.zeros((3, 6, len(bins_BL)-1, len(bins_BL)-1)) # (area_type, mcs_phase, BL)
    ...
    for p, phase in enumerate(["CCS","Init", "Grow", "Mature", "Decay", "End"]):
        ...
        # ===== for inside mcs ======
        BL_phase_mcs = data_BLenvmcs_phase.BL_TOT_mcs.sel(mcs_phase=phase) # 9.81/(340*3)*wb*((thetae_bl-thetae_sat_lt)/thetae_sat_lt)
        BL_phase_mcs = (340*3/9.81)*BL_phase_mcs # wb * (theta....)

        # get 1-D CAPE and SUBSAT values associated with MCS from all tracks
        BL_1d = BL_phase_mcs.values.ravel()
...
if __name__ == '__main__':
    ...
    data_merged_phase = xr.merge([data_buoy, data_mask])  
    print('total number of tracks in {}: {}'.format(year, len(data_merged_phase.tracks)))

    data_bins_merged = []
    for (dmin, dmax, duration_type) in zip([5,12,24,36],[12,24,36,72],
                                           ['SL','ML','LL','UL']):
    ...
    data_bins_duration = xr.concat(data_bins_merged, pd.Index(['SL','ML','LL','UL'], name='duration_type'))
