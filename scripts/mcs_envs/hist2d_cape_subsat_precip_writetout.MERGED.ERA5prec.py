...
def cape_subsat_hist(data_merged_phase):
    ...
    for p, phase in enumerate(["CCS","Init", "Grow", "Mature", "Decay", "End"]):
        ...
        prec = data_merged_phase.mtpr.sel(mcs_phase=phase).fillna(0)*3600 # converting from kg/m/s to mm/hr
        ...
        prec = data_merged_phase.mtpr.sel(mcs_phase=phase).fillna(0)*3600
        mcs_mask = data_merged_phase.cloudtracknumber_nomergesplit.sel(mcs_phase=phase).fillna(0)
        prec_mcs = prec.where(mcs_mask == 0)
        prec_1d = prec_mcs.values.ravel()
...
if __name__ == '__main__':
    ...
    prec_dir = Path('/scratch/wmtsai/temp_mcs/output_stats/buoy_meregd_2001_2020/{}/environment_catalogs/VARS_2D'.format(year))
    data_buoy = xr.open_dataset(buoy_dir / 'MCS_FLEXTRKR_tropics_buoyancy.merged.nc')
    data_mask = xr.open_dataset(mask_dir / 'MCS_FLEXTRKR_tropics_cloudtracknumber_nomergesplit.merged.nc').drop_vars(('lat','lon'))
    data_prec = xr.open_dataset(prec_dir / 'MCS_FLEXTRKR_tropics_mpr.merged.nc').drop_vars(('lat','lon'))
    ...
    data_bins_duration.to_netcdf(out_dir / 'hist2d_cape_subsat_dtype.{}.{}.precip.bins0.25.ERA5prec.nc'.format(year,sampling_opt))
    print('hist2d_cape_subsat_dtype.{}.{}.3deg.precip.ERA5prec.nc'.format(year,sampling_opt))
