...
# convection over Amazon [85W-30W, 22S-8N]
meanlat = data_track.meanlat.sel(times=0)
idx_lat = meanlat.where((meanlat > -22) & (meanlat < 8)).dropna(dim='tracks').tracks
meanlon = data_track.meanlon.sel(times=0)
idx_lon = meanlon.where((meanlon > -80) & (meanlon < -30)).dropna(dim='tracks').tracks
...
ds_tracks_merged.to_netcdf(dir_out / 'mcs_tracks_non2mcs_{}.AMAZON.nc'.format(year))
